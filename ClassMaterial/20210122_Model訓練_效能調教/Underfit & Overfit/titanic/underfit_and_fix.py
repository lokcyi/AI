...
train_history = model.fit(x=train_features, y=train_labels, validation_split=0.1, epochs=100, batch_size=30, verbose=2)  
...
all_df.insert(len(all_df.columns), 'probability', all_probability * 100) # 加入 生存機率 欄位
print("\n[Info] 預測結果 (傑克 & 蘿絲):\n%s\n" % (all_df[-2:]))
#print("\n[Info] 預測結果 (所有乘客):\n%s\n" % (all_df))
